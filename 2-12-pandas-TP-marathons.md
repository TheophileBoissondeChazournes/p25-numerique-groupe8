...
```{code-cell} ipython3
# votre code
df.to_csv('data/marathon_clean.csv', index=False)
```
...
```{code-cell} ipython3
# votre code
df2=pd.read_csv('data/marathon_clean.csv')
df2
```
...
```{code-cell} ipython3
# à vous
s1_paris_2000 = df['year'].loc[(df['city']=='PARIS')&(df['year']>=2000)]
```
...
```{code-cell} ipython3
# votre code
seconds_average = df['seconds'].describe()['mean']
```
...
```{code-cell} ipython3
# à vous
count_by_year = df.groupby(by='year')['year'].value_counts()
count_by_year
```
...
```{code-cell} ipython3
# votre code
count_by_city = df.groupby(by='city')['city'].value_counts()
count_by_city
```
...
```{code-cell} ipython3
# votre code
df.groupby('city')['seconds'].mean().sort_values()
```
