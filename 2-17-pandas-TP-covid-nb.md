...
```{code-cell} ipython3
# votre code
df_France = pd.DataFrame(by_country["France"])
df_France.head()
```
...
```{code-cell} ipython3
# votre code
print(df_France.shape[0]) #nombre de colonnes 
print(df_France.columns) #colonnes
```
...
```{code-cell} ipython3
# votre code
df_Italy = pd.DataFrame(by_country["Italy"])
...
# votre code
df_partiel=[]
for country, elem in by_country.items():
    df_c = pd.DataFrame(elem)
    df_c["country"] = country 
    df_partiel.append(df_c)
df_tot = pd.concat(df_partiel, ignore_index=True)   
df_tot.head()
```
...
```{code-cell} ipython3
# votre code
clean_df = df_tot.pivot_table(index=["country", "date"],
...
```{code-cell} ipython3
# votre code
df_fr_it = clean_df.loc[["France", "Italy"]]
df_fr_it.head()
df_fr_it.tail()
```
...
```{code-cell} ipython3
# votre code
df3 = clean_df.loc[["France", "Italy", "Germany"], ["confirmed", "deaths"]]
```
...
# 
# à vous
df3.unstack(level=0).plot(figsize=(12,6))
```
...
# votre code
def extract(countries, measures, start, end):
    df = clean_df.loc[countries, measures]
    df = df.loc[(slice(None), slice(start, end)), :]
    df = df.unstack(level=0)
    df.columns = ["_".join((m, p)) for (m, p) in df.columns]
    return df
```
...
```{code-cell} ipython3
# votre code
df_g = extract(countries=["France", "Italy"], measures=["confirmed", "deaths"], start="2020-01-01", end="2021-01-01")
df_g.plot()
```
...
```{code-cell} ipython3
# total de cas confirmé

total_confirmed = clean_df["confirmed"].groupby("country").max()
total_confirmed.sort_values(ascending=False)
```

```{code-cell} ipython3
# premier cas dans chaque pays 

first_case = {}

for country, df_country in clean_df.groupby("country"):
    df_non_zero = df_country[df_country["confirmed"] > 0]
    if len(df_nonzero) == 0:
        first_case[country] = None
    else:
        first_case[country] = df_nonzero.index.min()
...
```{code-cell} ipython3
# votre code
# Maximum de décès et date du pic en france

daily_deaths = df_fr["deaths"].diff().dropna()

pic_value = None
pic_date = None

for date, value in daily_deaths.items():

    if (pic_value is None) or (value > pic_value):
        pic_value = value
        pic_date = date

print("Pic de décès quotidiens :", pic_value)
print("Date du pic :", pic_date)
```

```{code-cell} ipython3
# Pour tous les pays (groupby)

df_deaths = clean_df["deaths"].groupby("country").diff().dropna()
pics = {}

for country, serie in df_deaths.groupby("country"):
    pic_value = None
    pic_date  = None
    for date, value in serie.items():
        if (pic_value is None) or (value > pic_value):
            pic_value = value
            pic_date  = date
    pics[country] = (pic_value, pic_date[1])

rows = []

for country, (value, date) in pics.items():
    rows.append([country, value, date])

df_pics = pd.DataFrame(rows, columns=["country", "pic_daily_deaths", "pic_date"])
df_pics = df_pics.set_index("country")
...
```{code-cell} ipython3
# votre code
# vagues (maximas locaux) ulisation rolling

df_fr_wave = df_fr["confirmed"].diff().rolling(7).mean()
df_fr_wave.plot(title="Nouveaux cas (moyenne 7 jours) - France");
```

```{code-cell} ipython3
# votre code
# ration death/confirmed

df_fr['fatality_rate'] = df_fr['deaths'] / df_fr['confirmed']
df_fr['fatality_rate'].plot(title="Taux de létalité (France)");
```

```{code-cell} ipython3
# votre code
# comparaison taux létalité entre France / Italy / Germany

df_lat = clean_df.loc[["France","Italy","Germany"], ["deaths","confirmed"]]
df_lat = (df_lat["deaths"] / df_lat["confirmed"]).unstack(0)

df_lat.plot(title="Comparaison du taux de létalité");
```
